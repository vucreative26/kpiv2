...
function payrollForm(payroll={}){
  ...
  const available=[
    ...d.subtasks.filter(s=>s.status==='done'||percent(progressForSubtask(s,d.tasks))===100).map(s=>({sourceType:'subtask',sourceId:s.id,title:s.title,meta:kpiById.get(s.kpiId)?.title||'Sub-task',amount:0})),
    ...d.urgent.filter(u=>u.status==='done').map(u=>({sourceType:'urgent',sourceId:u.id,title:u.title,meta:'Việc gấp đã hoàn thành',amount:0}))
  ];
  ...
  const entry=(item,index)=>`<div class="payroll-entry" data-source-key="${esc(`${item.sourceType}:${item.sourceId||item.title}`)}"><input type="hidden" name="sourceType" value="${esc(item.sourceType)}"><input type="hidden" name="sourceId" value="${esc(item.sourceId||'')}"><input class="pay-title" name="lineTitle" value="${esc(item.title)}" ${item.sourceType==='manual'?'':'readonly'}><input class="pay-amount" name="lineAmount" type="number" value="${Number(item.amount)||0}" step="1000"><button type="button" data-remove-pay aria-label="Xóa dòng">×</button></div>`;
  openModal(payroll.id?'Chỉnh sửa bảng lương':'Tạo bảng lương',`<div class="payroll-info">${input('title','Tên bảng lương',payroll.title||`Bảng lương ${S().period}`,'text','required')}${textarea('note','Ghi chú',payroll.note)}</div><div class="payroll-builder"><section><div class="payroll-pane-head"><div><h3>Các dòng lương</h3><p>Điều chỉnh tên và số tiền trước khi lưu.</p></div><button type="button" class="button subtle" id="addManualPay">+ Dòng khác</button></div><div id="selectedPayLines">${initial.map(entry).join('')}</div><div class="payroll-total"><span>Tổng dự kiến</span><b id="payrollDraftTotal">${money(initial.reduce((n,i)=>n+Number(i.amount||0),0))}</b></div></section><aside><div class="payroll-pane-head"><div><h3>Việc đã hoàn thành</h3><p>Bấm + để đưa vào bảng lương.</p></div></div><div class="payroll-source-list">${available.length?available.map((s,i)=>`<button type="button" data-add-pay-source="${i}"><span><b>${esc(s.title)}</b><small>${esc(s.meta)}</small></span><i>+</i></button>`).join(''):empty('Chưa có Sub-task hoặc việc gấp hoàn thành.')}</div></aside></div>`,async(_v,form)=>{
  ...
  $('#addManualPay').onclick=()=>append({sourceType:'manual',sourceId:null,title:'Khoản khác',amount:0});
  $('.payroll-source-list')?.addEventListener('click',e=>{const button=e.target.closest('[data-add-pay-source]');if(!button)return;const item=available[Number(button.dataset.addPaySource)],key=`${item.sourceType}:${item.sourceId}`;if(list.querySelector(`[data-source-key="${CSS.escape(key)}"]`))return toast('Dòng này đã có trong bảng lương.',true);append(item);button.disabled=true;});
